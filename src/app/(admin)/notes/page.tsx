...
import Note, { NoteProps as NoteType } from "@/components/ui/NoteBody";
    ...
    <>
      <main className="mt-10">
        <div className="bg-amber-400 p-2  rounded-lg shadow-xl flex  m-10 flex-col">
          <label htmlFor="countryFilter" className="font-bold mb-3">
            Filter By{" "}
          </label>
          <section className="flex gap-2 flex-col sm:flex-row ">
            <div className="flex  ">
              ...
              {selectedFilter.country && (
                <button
                  className=" font-bold m-2 px-2 rounded-full bg-blur-md bg-red-500 hover:bg-red-700"
                  onClick={() =>
            ...
            <div className="flex">
              <input
                ...
                className="p-2 rounded-md w-full  bg-gray-100 "
                onChange={(e) =>
                  setSelectedFilter({ ...selectedFilter, date: e.target.value })
                }
              />
              {selectedFilter.date && (
                <button
                  className="font-bold m-2 px-2 rounded-full bg-blur-md bg-red-500 hover:bg-red-700"
                  onClick={() =>
                    setSelectedFilter({ ...selectedFilter, date: "" })
                  }
                >
                  X
                </button>
          ...
          notesFiltered && notesFiltered.length > 0 ? (
            ...
            !charging && (
              <p className="m-10 font-bold">
                Doesn&apos;t exist notes with this filter
              </p>
            )
          )
        ) : (
...
const Paginator = memo(({ notes }: { notes: NoteType[] }) => {
  ...
  function Items({ currentItems }: { currentItems: NoteType[] }) {
    return (
      <>
        {currentItems &&
          currentItems.map((note, index) => <Note key={index} note={note} />)}
      </>
    );
  }
