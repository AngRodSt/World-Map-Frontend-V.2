...
export async function handleChangePassword(prevState: unknown, formData: FormData) {
    ...
    const errorChangePassword = await changePassword({ info: { currentPassword, newPassword } })
    if (errorChangePassword) {
        return { success: false, message: null, error: errorChangePassword };
    }
    return { success: true, message: 'Password Updated successully', error: null };
}