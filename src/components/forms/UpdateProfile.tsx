...
const initialState = { success: false, user: {}, error: null } as {
  success: boolean;
  user: Auth | null;
  error: string | null;
};
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-5 ">
          <div className="flex flex-col sm:flex-row items-center  gap-5">
            <div
              ...
              }  border w-[6rem] h-[6rem] transition-all ease-in-out duration-300  rounded-full shadow-xl bg-white overflow-hidden`}
            >
              <label
                htmlFor="avatar"
                className="w-[6rem] h-[6rem] flex justify-center items-center"
              >
                {avatarExist &&
                profile &&
                profile.avatar &&
                "data" in profile.avatar &&
                "contentType" in profile.avatar ? (
                  <img
                    src={`data:${profile.avatar.contentType};base64,${profile.avatar.data}`}
                  />
                ) : preview ? (
                  <img
                    src={preview}
                    ...
                    className={`${
                      edit ? "opacity-50" : ""
                    } hover:opacity-100 transition-all ease-out`}
                  />
                ) : (
                  <svg
          ...
          <button
            type="button"
            className="border   bg-amber-400 hover:bg-gray-700 p-4 rounded-full mr-0 sm:mr-4 hover:scale-110 transition-transform ease-in-out"
          ...
          <div className="w-full ">
            <div className="mb-4">
              ...
              <textarea
                ...
                disabled={edit ? false : true}
                className={`${
                  edit ? "bg-gray-50" : "bg-gray-300 text-gray-500"
                } border p-2 mt-3 w-full rounded-lg`}
                placeholder="Add Bio"
                autoComplete="bio"
                value={profile.bio}
                rows={2}
            ...
            <div className="flex justify-between sm:flex-row flex-col align-middle gap-10">
              <div className="w-full">
                ...
                <label htmlFor="birthDate" className="font-bold block mt-5">
                  {" "}
                  Birth Date{" "}
                </label>
                <input
              ...
              <div className="w-full">
                ...
                <label htmlFor="profession" className="font-bold mt-5 block">
                  {" "}
                  Profession{" "}
                </label>
                <input
            ...
            <div className="mt-10">
              {edit && (
                <Button text={"Save Changes"} setButtonClicked={isPending} />
              )}
