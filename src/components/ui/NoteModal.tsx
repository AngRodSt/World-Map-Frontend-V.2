    ...
    <>
      ...
      <div className="mt-5  ">
        <form onSubmit={handleSubmit} className="flex flex-col">
          <label htmlFor="name" className="font-bold text-xl text-gray-700">
            Name
          </label>
          <input
            value={noteLocal.name}
            onChange={(e) =>
              setNoteLocal({ ...noteLocal, name: e.target.value })
            }
            id="name"
            type="text"
            placeholder="Name of the person you met (not mandatory)"
            className="p-2 rounded-lg my-2 bg-gray-100 border"
          />
          <label htmlFor="messege" className="font-bold text-xl text-gray-700">
            Notes
          </label>
          <textarea
            value={noteLocal.message}
          ...
          <Button
            text={`${note._id !== "" ? "Edit Note" : "Add Note"}`}
